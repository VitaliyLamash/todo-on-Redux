...
export const List = ({ todos, handleChecked, handleDelete, typeShow, changeTodo }) => {
  const listTodo = todos.map(todo => {
    if (typeShow === "all") {
      return (
        ...
        >
		  {todo.todo}
		  <button onClick={() => changeTodo(todo.todo, todo.id)}>Change</button>
  ...
  return <TodoUl todos={todos} back={background}>{listTodo}</TodoUl>;
};
